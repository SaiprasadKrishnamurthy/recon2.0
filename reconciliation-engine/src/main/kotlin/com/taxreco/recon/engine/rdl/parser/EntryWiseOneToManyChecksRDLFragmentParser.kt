...
class EntryWiseOneToManyChecksRDLFragmentParser : RDLFragmentParser {

    override fun parse(lineNo: Int, line: String, setting: ReconciliationSetting): List<RDLParseError> {
        if (line.contains(RulesetType.EntryWiseOneToManyChecks.name)) {
            val template =
                ...
                if (groups.size != 6) {
                    ...
                    if (tokens.size != 7 && tokens.size != 4) {
                        return listOf(
                            RDLParseError(
                                lineNo,
                                "The one to many checks must be of the format: For example: 'sales.invoiceNo == tdsLedger.invoiceNo AND valueWithinTolerance(sales.tdsOnSales, tdsLedger.tds, 1.0)'.\n No complex expressions are allowed."
                            )
                        )
                    }
                    var rs =
                    ...
                    rs.name = name
                    if (type == RulesetType.EntryWiseOneToManyChecks) {
                        rs.type = RulesetType.EntryWiseOneToManyChecks
                        if (tokens.size == 7) {
                            rs.rules.add(
                                Rule(
